...
use crate::utils::{init_pda_account_rent, FixedSizeString, MAX_PADDED_STRING_LENGTH};
use crate::ID;
...
impl<'info> UpdateFolio<'info> {
    pub fn distribute_fees(
        ...
        let folio_token_mint: Box<InterfaceAccount<Mint>> = Box::new(InterfaceAccount::try_from(
        ...
        let index_for_fee_distribution_parsed = index_for_fee_distribution.unwrap().to_le_bytes();
        ...
        let fee_distribution: AccountLoader<FeeDistribution> = AccountLoader::try_from_unchecked(
            &system_program::ID,
            &remaining_accounts[IndexPerAccount::FeeDistribution as usize],
        )?;
...
pub fn handler<'info>(
    ...
    let mut are_fees_distributed = false;
    ...
    if let Some(scaled_tvl_fee) = scaled_tvl_fee {
        ...
        are_fees_distributed = true;

        ctx.accounts.folio.load_mut()?.set_tvl_fee(scaled_tvl_fee)?;
    }
