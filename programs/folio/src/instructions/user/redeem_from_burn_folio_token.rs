...
pub fn handler<'info>(
    ...
    folio_pending_token_amounts.remove_token_amounts_from_folio(
        &removed_mints,
        false,
        PendingTokenAmountsType::RedeemProcess,
    )?;

    let user_pending_token_amounts = &mut ctx.accounts.user_pending_token_amounts.load_mut()?;
    user_pending_token_amounts.remove_token_amounts_from_folio(
        &removed_mints,
        true,
        PendingTokenAmountsType::RedeemProcess,
    )?;
