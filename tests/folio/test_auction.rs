...
mod tests {
    ...
    use folio::state::Auction;
    use folio::utils::structs::AuctionStatus;
    use folio::utils::{BasketRange, PricesInAuction};
    ...
    fn test_validate_auction_approve() {
        ...
        let invalid_prices = PricesInAuction {
            start: 500,
            end: 1000,
        };
        assert_eq!(
    ...
    //         high: 150,
    //     };
    //     auction.initial_proposed_price = PricesInAuction {
    //         start: 1000,
    //         end: 500,
    //     };
    ...
    //         id: 1,
    //         initial_proposed_price: PricesInAuction {
    ...
    fn test_calculate_k() {
        ...
        let k = auction_run.calculate_k().unwrap();
        // ln(2)/3600 ≈ 192,540,883,488,873.6970603423 (in D18)
        let expected_k = 192_540_883_488_873u128;

        assert!(k >= expected_k - 1000u128 && k <= expected_k + 1000u128);
        ...
        let k = auction_run.calculate_k().unwrap();
        assert_eq!(k, 0); // ln(1) = 0, so k should be 0
    ...
    fn test_get_price() {
        ...
        let auction_run_details = Auction {
            start: now,
            end: end_time,
            prices: PricesInAuction {
                start: 10000000000000000000u128, // 10 * D18
